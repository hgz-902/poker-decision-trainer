...
function actionBadgeStyle(text: string): React.CSSProperties {
  ...
  if (t.startsWith("POST")) return { ...base, background: "rgba(0, 150, 136, 0.92)" };
  if (t.startsWith("FOLD")) return { ...base, background: "rgba(120,120,120,0.9)" };
  if (t.startsWith("CALL") || t.startsWith("CHECK"))
    return { ...base, background: "rgba(25,118,210,0.9)" };
  if (t.startsWith("RAISE") || t.startsWith("BET"))
    return { ...base, background: "rgba(245,124,0,0.92)" };
...
export default function PokerTable({
  ...
  return (
    // ✅ outer는 "레이아웃 기준 박스"
    <div
      className="pdt-table-outer"
      ref={outerRef}
      style={{
        width: "100%",
        // 혹시 CSS가 꼬여있어도 안전하게 레이아웃을 고정
        position: "relative",
      }}
    >
      {/* ✅ spacer가 "실제로 차지하는 높이" (겹침 방지의 핵심) */}
      <div
        ...
        style={{
          ...
          position: "relative",
          overflow: "hidden", // ✅ scale된 요소가 튀어나와도 아래를 침범 못하게
        }}
      >
        {/* ✅ wrap은 absolute + scale (transform은 레이아웃을 밀지 않기 때문에 spacer가 필요) */}
        <div
          ...
          style={{
            ...
            transformOrigin: "top left", // ✅ 기준점을 고정 (침범/흔들림 방지)
          }}
        >
          {/* -----------------------------
              기존 테이블 렌더링 부분 (그대로)
          ...
          <div style={{ border: "1px solid #ddd", borderRadius: 14, padding: 12, overflow: "hidden" }}>
            ...
            <div
              style={{
                ...
                borderRadius: 16,
                background:
                  "radial-gradient(circle at 50% 40%, #1f6a3a 0%, #0f3e25 55%, #0b2c1a 100%)",
